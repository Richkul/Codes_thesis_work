# -*- coding: utf-8 -*-
"""
Created on Thu Nov 26 15:08:57 2020
...
def Average(data, Parameter, Type="Normal"):
    '''
    Computes the Average

    - data: xarray dataframe with the variables units defined (required)
    - Parameter: the input parameter name (required)
    - Type: the type of averaging to conduct (optional)
    '''

    if Type == "Normal":
        AVG = np.mean(data[Parameter], axis=0)
    else:
        # The nanmean function is used for solar.
        AVG = np.nanmean(data[Parameter], axis=0)

    AVG = AVG.data.reshape(1, AVG.shape[0], AVG.shape[1])

    AVG = xr.DataArray(AVG, dims=('time', 'latitude', 'longitude',))
    AVG.name = Parameter
    AVG.attrs['units'] = data[Parameter].units
